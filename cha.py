...
class ChallengeEvent:
	...
	def Rename_existing_replaypack(self: "ChallengeEvent", compress:bool) -> None:
		if not self.has_replays:
			return
			
		existing = ChaSys.chareps / f"torename{self.id}.rar" 
		if existing.exists() and not self.replays_dir.exists():
			existing.rename(ChaSys.chareps/self.replays_dir.name)
			print(f"* {existing.name} was renamed to {self.replays_dir.name}")
			return
			
		existing = ChaSys.chareps / f"torename{self.id}.zip" 
		if existing.exists() and not self.replays_dir.exists():
			existing.rename(ChaSys.chareps/self.replays_dir.name)
			print(f"* {existing.name} was renamed to {self.replays_dir.name}")
			return
	...
	def preguntar_por_replaypack(self: "ChallengeEvent") -> None:
		while self.has_replays and not self.replays_dir.exists():
			if not get_boolean(f"Replay pack not found: << {self.replays_dir.relative_to(self.replays_dir.parent.parent)} >> \n{self.replays_dir.stem}\n\tDo you want to make sure to rename replays accordingly and try again?"):
				sys.exit("Ok bye")
			self.Rename_existing_replaypack(compress=False)
...
class ChallengeSystem:
	...
	def rename_last_n_replaypacks(self: "ChallengeSystem", howmany: int) -> None:
		for challenge in BaseDeDatos.CHALLENGES[:-howmany:-1]:
			challenge.Rename_existing_replaypack(compress=False)
	...
	def execute_argv_operations_if_any(self: "ChallengeSystem", argv: list[str]) -> None:
		...
		if len(argv) > 1:
			...
			argv_dict["chaId"] = get_int("Type challenge id: ", min=min_chall, max=max_chall)
